#https://docs.ray.io/en/master/tune/tutorials/tune-pytorch-cifar.html#communicating-with-ray-tune
...
from common.cmd_args import CIFAR_100_PATH

def load_data(data_dir="./data"):
    #TODO: We wrap the data loaders in their own function and pass a global data directory.
...
class Net(nn.Module):
    #TODO: We can only tune those parameters that are configurable.
...
def train_cifar(config, checkpoint_dir=None, data_dir=None):
    ...
    for epoch in range(10):  # loop over the dataset multiple times
        ...
        # TODO: Here we first save a checkpoint and then report some metrics back to Tune.
        #  Specifically, we send the validation loss and accuracy back to Tune.
        #  Tune can then use these metrics to decide which hyperparameter configuration lead to the best results.
...
def test_accuracy(net, device="cpu", data_dir='./data'):
    trainset, testset = load_data(data_dir)
...
def main(num_samples=10, max_num_epochs=10, gpus_per_trial=2):
    data_dir = CIFAR_100_PATH#os.path.abspath("./data")
    load_data(data_dir)  # Download data for all trials before starting the run
    #TODO: define Tune’s search space
    ...
    #TODO: We wrap the train_cifar function with functools.partial to set the constant data_dir parameter.
    ...
    # Trials do not have access to GPUs that haven’t been requested for them - so you don’t have to care about two trials using the same set of resources.
    ...
    test_acc = test_accuracy(best_trained_model, device, data_dir = data_dir)
    print("Best trial test set accuracy: {}".format(test_acc))

if __name__ == "__main__":
    main(num_samples=1, max_num_epochs=10, gpus_per_trial=4)