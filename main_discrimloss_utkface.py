...
os.environ["CUDA_VISIBLE_DEVICES"] = "2"#"0,1,2,7"#
...
def validate(args, val_loader, model, criterion, epoch):
    ...
    with torch.no_grad():
        for (inputs, target, _) in tqdm(val_loader, desc="Evaluating"):
            # if args.device == 'cuda':
            #     inputs = inputs.cuda()
            #     target = target.cuda()
...
def train_for_one_epoch(args,
    ...
    for i, (inputs, target, index_dataset) in enumerate(epoch_iterator):
...
def main_worker(args, config):
    ...
    start_epoch_time = time.time()
    train_iterator = trange(int(args.start_epoch), int(args.epochs), desc="Epoch", disable=args.local_rank not in [-1, 0])
...
def main():
    ...
    args.log_dir = CURRENT_PATH + '/UTKFACE/discrimloss_results/logs_n80_bt128_cn1_ea_emak_tanh_wd6_lr1e-3_lrin1e-4_L2'
    args.save_dir = CURRENT_PATH + '/UTKFACE/discrimloss_results/weights_n80_bt128_cn1_ea_emak_tanh_wd6_lr1e-3_lrin1e-4_L2'
...
nohup python main_discrimloss_utkface.py >> ./UTKFACE/discrimloss_results/log_n60_bt128_cn1_ea_emak_tanh_wd6_lr0.1_L1.log 2>&1 &
